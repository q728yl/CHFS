...
//      UNIMPLEMENTED();
                auto idx = free_idx.value() + count * this->bm->block_size() * KBitsPerByte;
             //   std::cout<<"metadata的managercc分配到的inode"<<RAW_2_LOGIC(idx)<<std::endl;
                Inode newInode = Inode(type, this->bm->block_size());
                this->bm->write_partial_block(bid, (u8 *) &newInode, 0, sizeof(newInode));
                set_table(idx, bid);
...
// { Your code here }
    auto InodeManager::set_table(inode_id_t idx, block_id_t bid) -> ChfsNullResult {
        ...
        // whose index is `idx`.
//        UNIMPLEMENTED();
        auto inodeNumPerBlock = this->bm->block_size() / sizeof(block_id_t);
        auto entryIdx = idx / inodeNumPerBlock;
//        std::cout<<"entryIdx: "<<entryIdx<<std::endl;
        auto entryOffset = idx % inodeNumPerBlock;
        entryOffset *= sizeof(block_id_t);
       // std::cout<<"entryIdx: "<<entryIdx<<"entryOffset: "<<entryOffset<<std::endl;
        this->bm->write_partial_block(RAW_2_LOGIC(entryIdx), (u8 *) &bid, entryOffset, sizeof(block_id_t));
        return KNullOk;
    }
...
//前几步同上
        auto inodeNumPerBlock =  this->bm->block_size() / sizeof(block_id_t);
        auto entryIdx = LOGIC_2_RAW(id) / inodeNumPerBlock;
        auto entryOffset = LOGIC_2_RAW(id) % inodeNumPerBlock * sizeof(block_id_t);
        std::vector<u8> buffer(this->bm->block_size());
        this->bm->read_block(1 + entryIdx, (u8 *) buffer.data());//superBlock+1
        res_block_id = *(block_id_t *) (buffer.data() + entryOffset);//在实际位置的偏移量上按长度取出该值
...
//        UNIMPLEMENTED();
        auto inodeNumPerBlock = this->bm->block_size()  / sizeof(block_id_t);
        auto entryIdx = LOGIC_2_RAW(id) / inodeNumPerBlock;
        auto entryOffset = LOGIC_2_RAW(id) % inodeNumPerBlock;
        entryOffset *= sizeof(block_id_t);
        this->bm->write_partial_block(1 + entryIdx, (u8 *) &KInvalidBlockID, entryOffset, sizeof(block_id_t));
//bitmap清零
        auto bitmapBlockIdx = LOGIC_2_RAW(id) / (this->bm->block_size() * KBitsPerByte);
        auto bitmapBlockOffset = LOGIC_2_RAW(id) % (this->bm->block_size() * KBitsPerByte);
        std::vector<u8> buffer(this->bm->block_size());
        //读+改部分+写回
        this->bm->read_block(1 + n_table_blocks + bitmapBlockIdx, buffer.data());
        auto partOfBitmap = Bitmap(buffer.data(), this->bm->block_size());;
        partOfBitmap.clear(bitmapBlockOffset);
        this->bm->write_block(1 + n_table_blocks + bitmapBlockIdx, buffer.data());
        return KNullOk;
    }
