...
//        UNIMPLEMENTED();
        ...
        this->block_data =
                static_cast<u8 *>(mmap(nullptr, this->total_storage_sz(), PROT_READ | PROT_WRITE, MAP_SHARED, this->fd,
                                       0));
        CHFS_ASSERT(this->block_data != MAP_FAILED, "Failed to mmap the data");
        if (is_log_enabled) {
            this->block_cnt -= 1025;//一个bitmap就够，加上1024个log的block
        }
    }
    //log看上去是物理备份，直接copy内容的
    auto BlockManager::write_block(block_id_t block_id, const u8 *data)
    -> ChfsNullResult {
        if (this->maybe_failed && block_id < this->block_cnt) {
            std::vector<u8> copyData(DiskBlockSize);
            memcpy(copyData.data(), data, DiskBlockSize);
            auto op=std::make_shared<BlockOperation>(block_id, copyData);
            CommitLog::ops_.emplace_back(op);
            if (this->write_fail_cnt >= 3) {
                CommitLog::txnFail = true;
                this->write_fail_cnt = 0;
        ...
        // TODO: Implement this function.
//  UNIMPLEMENTED();
//        CHFS_ASSERT(block_id < block_cnt, "ID out of range");
//        CHFS_ASSERT(data != nullptr, "Invaild pointer");

        u64 offset = static_cast<u64>(block_id) * block_sz;
    ...
    -> ChfsNullResult {
        if (this->maybe_failed && block_id < this->block_cnt) {
            std::vector<u8> copyData(block_sz);
            read_block(block_id, copyData.data());
            memcpy(copyData.data() + offset, data, len);
            auto op=std::make_shared<BlockOperation>(block_id, copyData);
            CommitLog::ops_.emplace_back(op);
            if (this->write_fail_cnt >= 3) {
                CommitLog::txnFail = true;
...
//        CHFS_ASSERT(block_id < block_cnt, "ID out of range");
//        CHFS_ASSERT(data != nullptr, "Invaild pointer");
        CHFS_ASSERT(offset + len <= block_sz, "beyond len");

        u64 block_offset = static_cast<u64>(block_id) * block_sz;
        u8 *block_ptr = block_data + block_offset;

        memcpy(block_ptr + offset, data, len);
        this->write_fail_cnt++;
        return KNullOk;
    }
