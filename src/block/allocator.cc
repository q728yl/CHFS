...
// Your implementation
    auto BlockAllocator::allocate() -> ChfsResult<block_id_t> {
        ...
        for (usize i = 0; i < this->bitmap_block_cnt; i++) {
            ...
            if (res) {
                auto bei = i * KBitsPerByte * bm->block_size();
                block_id_t retval = bei + *res;
...
// Your implementation
    auto BlockAllocator::deallocate(block_id_t block_id) -> ChfsNullResult {
        if (block_id >= this->bm->total_blocks()) {
            std::cout << "zai1" << std::endl;
            return ChfsNullResult(ErrorType::INVALID_ARG);
        }

        usize bitmapBlockIndex = (block_id ) / (KBitsPerByte * bm->block_size());//bitmap中的哪个块(0开始算的话)
        usize bitOffset = (block_id ) % (KBitsPerByte * bm->block_size());//bitmap中的哪个位（在每个块中从0开始数）

        std::vector<u8> buffer(bm->block_size());
        bm->read_block(bitmapBlockIndex + this->bitmap_block_id, buffer.data());

        Bitmap bitmap(buffer.data(), bm->block_size());
        if (!bitmap.check(bitOffset)) {
            std::cout<<"zai2"<<std::endl;
            return ChfsNullResult(ErrorType::INVALID_ARG);
        }
        bitmap.clear(bitOffset);
        //设置
        bm->write_block(bitmapBlockIndex + this->bitmap_block_id, buffer.data());
//        std::cout<<"kNullOKdeisOk"<<KNullOk.is_ok()<<std::endl;
        return KNullOk;
    }
} // namespace chfs
